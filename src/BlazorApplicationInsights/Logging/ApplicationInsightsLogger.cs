...
/// <remarks>Initializes a new instance of the <see cref="ApplicationInsightsLogger"/> class</remarks>
...
public class ApplicationInsightsLogger(string? categoryName, IApplicationInsights applicationInsights) : ILogger {
	...
	public void Log<TState>(LogLevel logLevel, EventId eventId, TState state, Exception? exception, Func<TState, Exception?, string> formatter) {
		...
		var customDimensions = this.GetCustomDimensions(state, eventId);

		if (exception is null) {
			this._applicationInsights.TrackTrace(new TraceTelemetry() { Message = message, SeverityLevel = severity, Properties = customDimensions });
			return;
		}
		...
		this._applicationInsights.TrackException(new() { Exception = error, Id = $"{eventId}", SeverityLevel = severity, Properties = customDimensions });
	}
...
#pragma warning disable 618
		this.EnrichmentCallback(result);
#pragma warning restore 618

		this.ApplyScopes(result);
		this.ApplyLogState(result, state, eventId);
		return result;
	}

	[MethodImpl(MethodImplOptions.AggressiveInlining)]
	private void ApplyLogState<TState>(Dictionary<string, object?> customDimensions, TState state, EventId eventId) {
		if (this.IncludeCategoryName && !string.IsNullOrEmpty(this._categoryName)) {
			customDimensions["CategoryName"] = this._categoryName;
		}
	...
	private void ApplyScopes(Dictionary<string, object?> customDimensions) {
		...
		this.ScopeProvider.ForEachScope(ApplyScope, (customDimensions, scopeBuilder));
